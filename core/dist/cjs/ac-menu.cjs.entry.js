...
const AcMenu = class {
  constructor(hostRef) {
    ...
    this.menuChange = index.createEvent(this, "menuChange", 7);
  ...
  __stencil_render() {
    return [
      // Searchbar
      this.searchable &&
        index.h("ac-input-base", { class: "ac-menu__search-bar", size: "small", placeholder: this.searchLabel || 'Search', onKeyUp: this.handleDebouncedKeyUp, ref: searchInput => this.searchInput = searchInput }, index.h(acFaIcon.AcFaIcon, { slot: "item-start", icon: index_es.faSearch, size: 12 })),
      // Content
      index.h("div", { class: "ac-menu__content", ref: menuContentElt => this.menuContentElt = menuContentElt }, index.h("slot", null)),
      // Search Results
      this.filteredItems && this.filteredItems.length > 0 &&
        index.h("div", { class: "ac-menu__search-results" }, this.filteredItems.map(renderFilteredGroup))
    ];
  }
...
function renderFilteredGroup(group) {
  return (index.h("div", { class: "ac-menu__group" }, index.h("span", { class: "ac-menu__group-title" }, group.groupTitle), group.items.map(item => (index.h("ac-menu-item", { href: item.href, onClick: item.onClick }, item.title)))));
}
AcMenu.style = acMenuCss;

exports.ac_menu = AcMenu;

//# sourceMappingURL=ac-menu.cjs.entry.js.map